...
import com.huaxing.util.NgtDefine.ScheduleStatus;
...
public class ScheduleUtils {
	...
	public static void createScheduleJob(Scheduler scheduler, ScheduleJobEntity scheduleJob) {
		try {
			...
			CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity(getTriggerKey(scheduleJob.getJobId())).withSchedule(scheduleBuilder)
					.build();

			// 放入参数，运行时的方法可以获取
			jobDetail.getJobDataMap().put(ScheduleJobEntity.JOB_PARAM_KEY, JsonUtil.bean2Json(scheduleJob));
	...
	public static void updateScheduleJob(Scheduler scheduler, ScheduleJobEntity scheduleJob) {
		try {
			...
			// 参数
			trigger.getJobDataMap().put(ScheduleJobEntity.JOB_PARAM_KEY, JsonUtil.bean2Json(scheduleJob));
	...
	public static void run(Scheduler scheduler, ScheduleJobEntity scheduleJob) {
		try {
			...
			dataMap.put(ScheduleJobEntity.JOB_PARAM_KEY, JsonUtil.bean2Json(scheduleJob));
